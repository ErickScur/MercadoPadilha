...
<html lang="en">
  ...
  <body>
    ...
    <div class="container">
    <h1 class="display-3">Vendas cartão</h1>
    ...
    <table border="1" width="800" class="table-hover table-bordered table-striped">
		...
		<?php
		$sql = $pdo->prepare("SELECT * FROM cartao");
		$sql->execute();
		...
		if($sql->rowCount() > 0) {
			foreach($sql->fetchAll() as $item) {
				...
				<tr>
                    ...
                    <td>
                        <?php $id = $item['id'] ;
                        echo '<a href="editarCartao.php?id='.$id.'">Editar</a> <a href="excluirCartao.php?id='.$id.'">Excluir</a>'
                        ?>
                    </td>
                </tr>
    ...
    <a name="" id="" class="btn btn-primary" href="cadastrarCartao.php" role="button">Inserir vendas cartão</a>
    <br>
    <br>
    <br>
    <br>
    ...
    <a name="" id="" class="btn btn-danger" role="button" onclick="confirmar(2)">Apagar tudo</a>
    </div>
